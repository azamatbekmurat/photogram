...
const UserList: React.FunctionComponent<IUserListProps> = (props) => {
  ...
  const getAllSuggestedUsers = async (userId: string) => {
    ...
    return suggestedUser.map((user) => {
      return (
        <div className='flex flex-row items-center mb-4 border-gray-400 justify-start'>
          <span className='mr-2'>
            <img src={user?.photoURL ? user?.photoURL : avatar} className='w-10 h-10 rounded-full border-2 border-slate-800 object-cover' />
          </span>
